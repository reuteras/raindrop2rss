...
(function () {
  ...
  function transformFeed () {
    ...
    for (let i = 0; i < sortedEntries.length; i++) {
      ...
      if (summary) {
        // Safe approach: handle cover images from raindrop2rss while preventing XSS
        // The summary format from raindrop2rss.py is: <img src="cover_url"/><br/>note_text
        // We'll parse this specific safe format without using innerHTML
        
        const imgMatch = summary.match(/^<img src="([^"]+)"[^>]*\/>(<br\/>)?(.*)$/s)
        if (imgMatch && imgMatch[1] && imgMatch[3]) {
          // Only process if we have both image URL and text content
          // This matches the exact format generated by raindrop2rss.py
          
          // Create image element (safe because URL comes from trusted source)
          ...
          // Add text content safely using textContent (auto-escapes)
          if (imgMatch[3].trim()) {
          ...
          // No image format detected, treat as plain text
          const summaryPara = html('p')
